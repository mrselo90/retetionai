...
export async function scheduleUserMessage(
  ...
  // Idempotency: skip if a pending/completed task already exists for this order + type
  if (options.orderId) {
    const { data: existing } = await serviceClient
      ...
      .eq('task_type', options.messageType)
      .in('status', ['pending', 'completed'])
      .limit(1)
      .maybeSingle();

    if (existing) {
      return { taskId: existing.id, jobId: 'already_scheduled' };
    }
  }
  ...
  // Stable jobId prevents duplicate BullMQ jobs for the same order + type
  const stableJobId = options.orderId
    ? `${options.messageType}-${options.orderId}`
    : `${options.messageType}-${options.userId}-${options.scheduledFor.getTime()}`;
