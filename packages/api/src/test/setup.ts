    ...
    public chat = {
      completions: {
        create: vi.fn().mockImplementation(async (args: any) => {
          ...
          const isNegative = ['terrible', 'hate', 'not satisfied', 'bad', 'awful', 'worst'].some(
            ...
            choices: [
              {
                message: {
                  role: 'assistant',
                  content: wantsJson ? JSON.stringify(json) : 'Test AI response',
                },
              },
            ],
...
const defaultFetch = vi.fn(async (input: any) => {
  ...
  if (typeof url === 'string' && url.includes('/admin/oauth/access_token')) {
    return new Response(
      JSON.stringify({ access_token: 'test-access-token', scope: 'read_orders' }),
      {
        status: 200,
