...
function normalizeDashboardStats(
  input: Partial<DashboardStats> | null | undefined
): DashboardStats {
  return {
    ...
    knowledgeHealth: {
      ...
      topAtRiskProducts:
        input?.knowledgeHealth?.topAtRiskProducts ||
        DEFAULT_STATS.knowledgeHealth.topAtRiskProducts,
    },
    recentActivity: {
      orders: input?.recentActivity?.orders || DEFAULT_STATS.recentActivity.orders,
      conversations:
        input?.recentActivity?.conversations || DEFAULT_STATS.recentActivity.conversations,
    },
...
export default function DashboardPage() {
  ...
  // Stats failing to load falls back to all-zero defaults. Those defaults carry
  // no alerts, which reads as "Shopify connected", so without this flag a
  // failed request marked the first setup step done.
  ...
  const loadDashboard = useCallback(async () => {
    try {
      ...
      toast.error(
        t('toasts.dashboardLoadFailed.title'),
        getErrorMessage(err, t('toasts.dashboardLoadFailed.message'))
      );
    } finally {
      setLoading(false);
    }
  ...
  // already ask for, with their own button. Repeating them in a banner above
  // it showed the same call to action twice on a new account.
  const alertCoveredBySetup = (alert: DashboardStats['alerts'][number]) =>
    Boolean(nextStep) && (alert.type === 'no_integration' || alert.type === 'no_products');
  const visibleAlerts = displayStats.alerts.filter((alert) => !alertCoveredBySetup(alert));
  const topAlert =
    visibleAlerts.find((a) => a.severity === 'error' || a.severity === 'warning') ??
    visibleAlerts[0];
  ...
  const alertMessage = (alert: DashboardStats['alerts'][number]) => {
    ...
    if (alert.type === 'integration_error')
      return t('alerts.messages.integrationError', { provider: alert.provider ?? '' });
  ...
  return (
    <div style={{ maxWidth: 1100, display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <h1 className="r-page-title">{t(greetingKey, { name: merchant.name || 'Merchant' })}</h1>
        {!statsFailed && hasConversationActivity ? (
          <p className="r-page-sub">
            {displayStats.kpis.activeUsers ?? 0} {t('kpi.activeCustomers')} ·{' '}
            {displayStats.kpis.responseRate ?? 0}% {t('kpi.replyRate')}
          </p>
        ) : null}
      </div>
      ...
      {statsFailed ? (
        <div className="r-alert r-alert-error">
          ...
          <div style={{ flex: 1 }}>
            <p className="r-alert-body">{t('statsUnavailable')}</p>
            <Button
              variant="secondary"
              size="sm"
              onClick={() => {
        ...
        <div
          className={`r-alert r-alert-${topAlert.severity === 'error' ? 'error' : topAlert.severity === 'warning' ? 'warning' : 'info'}`}
        >
          <AlertTriangle size={16} aria-hidden="true" style={{ flexShrink: 0, marginTop: 2 }} />
          <div style={{ flex: 1 }}>
            <p className="r-alert-title">{t('alerts.title')}</p>
      ...
      {/* Getting started — only while there is something left to do. Once all
          three steps are done it was a permanent "3/3 completed" card at the
          top of the page. */}
      {!statsFailed && nextStep ? (
        <div className="r-card">
          ...
          >
            ...
            <Badge tone={nextStep ? 'caution' : 'success'}>
          ...
          <div className="r-stepper" style={{ marginBottom: nextStep ? 16 : 0 }}>
            {setupSteps.map((step, index) => (
              <div key={step.id} className="r-stepper-item">
                ...
                <div
                  className={`r-stepper-circle${step.completed ? ' done' : nextStep?.id === step.id ? ' current' : ''}`}
                >
                  {step.completed ? (
                ...
                <p
                  className="r-hint"
                  style={{ textAlign: 'center', marginTop: 6, padding: '0 4px' }}
                >
                  {step.title}
                </p>
              </div>
          ...
          {nextStep ? (
            <div
              style={{
                ...
                padding: 'var(--r-space-7) var(--r-space-8)',
                display: 'flex',
                justifyContent: 'space-between',
                alignItems: 'center',
                gap: 16,
            ...
            >
              ...
              <Link
                href={nextStep.actionUrl as '/dashboard/integrations'}
      ...
      {/* Knowledge health — needs products to score. On a new account it was a
          red "0" average with three empty tiles. */}
      {!statsFailed && hasProducts ? (
        <div className="r-card">
          <div
            style={{
              display: 'flex',
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span
                style={{
                  fontSize: 'var(--r-text-4xl)',
                  fontWeight: 'var(--r-weight-bold)',
                  letterSpacing: 'var(--r-tracking-tight)',
                }}
              >
          ...
          <div
            style={{
              ...
              gap: 12,
              marginBottom: displayStats.knowledgeHealth.productsAtRisk > 0 ? 14 : 0,
            }}
          >
            {[
              {
                label: t('knowledge.productsAtRisk'),
                value: displayStats.knowledgeHealth.productsAtRisk,
                hint: t('knowledge.productsAtRiskHint'),
              },
              {
                label: t('knowledge.strongProducts'),
                value: displayStats.knowledgeHealth.strongProducts,
                hint: t('knowledge.strongProductsHint'),
              },
              {
                ...
                hint:
                  displayStats.knowledgeHealth.topMissingReasonCount > 0
                    ? t('knowledge.primaryGapHint', {
                        count: displayStats.knowledgeHealth.topMissingReasonCount,
                      })
              ...
              >
                ...
                <p
                  style={{
                    margin: '0 0 4px',
                    fontSize:
                      typeof item.value === 'number' ? 'var(--r-text-2xl)' : 'var(--r-text-md)',
                    fontWeight: 'var(--r-weight-bold)',
                    letterSpacing:
                      typeof item.value === 'number' ? 'var(--r-tracking-tight)' : 'normal',
                  }}
                >
                  {item.value}
                </p>
          ...
          {displayStats.knowledgeHealth.productsAtRisk > 0 ? (
            <div className="r-alert r-alert-warning" style={{ marginTop: 0 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <Link href="/dashboard/products" className="r-btn r-btn-secondary r-btn-sm">
                  {t('knowledge.warningAction')}
                </Link>
              </div>
            </div>
          ) : null}
      ...
      {/* KPIs — hidden when stats failed, where they would all read 0. */}
      {!statsFailed ? (
        <div className="r-kpi-grid">
          {[
            {
              label: t('kpi.ordersReceived'),
        ...
        <div className="r-card">
          ...
          {displayStats.recentActivity.orders.length > 0 ? (
            ...
            <EmptyState
              ...
              action={
                <Link
                  href={ordersEmptyActionUrl as '/dashboard/integrations'}
                  className="r-btn r-btn-secondary r-btn-sm"
                >
                  {ordersEmptyActionLabel}
                </Link>
              }
            />
          )}
        ...
        <div className="r-card">
          ...
          {displayStats.recentActivity.conversations.length > 0 ? (
            ...
            <EmptyState
              ...
              action={
                <Link
                  href={conversationsEmptyActionUrl as '/dashboard/products'}
                  className="r-btn r-btn-secondary r-btn-sm"
                >
                  {conversationsEmptyActionLabel}
                </Link>
              }
            />
          )}
          ...
          {[
            {
              ...
              desc: t('quickActions.addProductDesc'),
            },
            {
              href: '/dashboard/integrations',
              icon: <Zap size={18} aria-hidden="true" />,
              title: t('quickActions.integration'),
              desc: t('quickActions.integrationDesc'),
            },
            {
            ...
            <Link
              key={i}
              href={action.href as '/dashboard/products'}
              className="r-card r-quick-action"
            >
              <span className="r-kpi-icon" style={{ flexShrink: 0 }}>
                {action.icon}
              </span>
